...
public class GuiMachineRecycler extends GuiMachineBase {
    ...
    protected void drawGuiContainerForegroundLayer() {
        ...
        drawStringCenteredNoShadow(fontRenderer, i18n.translateKey("gui.industry.t1.recycler"), scrnX, 6, 4210752);
    ...
    public void clickInventory(int x, int y, int mouseButton) {
        ...
        if (this.inventorySlots instanceof ContainerMachineBase) { // This is the only section that actually really matters
            if (Item.itemsList[clickedItemId] instanceof ItemEnergyContainer)
                target = 1;
            else if (stackInSlot != null && !stackInSlot.getItem().hasTag(IndustryTags.PREVENT_ITEM_RECYCLING))
                target = 2;
        }
