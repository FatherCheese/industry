...
import cookie.industry.block.energy.generator.entity.TileEntityGeneratorGeothermal;
...
public class GuiGeneratorGeothermal extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float f) {
        ...
        drawTexturedModalRect(scrnX + 80, (scrnY + 17) + (16 - fuelTime), 192, 24 - fuelTime, 16, fuelTime);
    }

    @Override
    protected void drawGuiContainerForegroundLayer() {
        int scrnX = xSize / 2;
        drawStringCenteredNoShadow(fontRenderer, i18n.translateKey("gui.industry.generator.geothermal"), scrnX, 6, 4210752);
    ...
    public void drawScreen(int x, int y, float renderPartialTicks) {
        ...
        if (x > (scrnX + 83) && x < (scrnX + 93))
            if (y > (scrnY + 20) && y < (scrnY + 31)) {
                String text = TextFormatting.WHITE + i18n.translateKey("gui.industry.lava") + ": " + TextFormatting.LIGHT_GRAY + tileEntity.currentFuelTime + TextFormatting.WHITE + " / " + TextFormatting.LIGHT_GRAY + tileEntity.maxFuelTime;
