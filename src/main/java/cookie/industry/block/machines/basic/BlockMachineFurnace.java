...
public class BlockMachineFurnace extends BlockTileEntityRotatable {
    private boolean keepInventory = false;
    private final String MOD_ID = Industry2.MOD_ID;
    private final int[][] machineTexture = new int[][]{
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_furnace.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_furnace_on.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_casing_basic.png")
    };

    public BlockMachineFurnace(String key, int id, Material material) {
    ...
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        if (!world.isClientSide) {
            TileEntityMachineFurnace tileEntity = (TileEntityMachineFurnace) world.getBlockTileEntity(x, y, z);
    ...
    public int getBlockTexture(WorldSource blockAccess, int x, int y, int z, Side side) {
        ...
        */
        ...
        TileEntityMachineFurnace tileEntity = ((TileEntityMachineFurnace) blockAccess.getBlockTileEntity(x, y, z));
    ...
    private static BlockMachineFurnace instance = null;

    private static void setupInstance(BlockMachineFurnace machine) {
        instance = machine;
    }

    private static BlockMachineFurnace getInstance() {
        if (instance == null)
            throw new NullPointerException("Instance of BlockMachineFurnace hasn't been setup!");
        return instance;
    }
