...
public class BlockMachineTrommel extends BlockTileEntityRotatable {
    private boolean keepInventory = false;
    private final String MOD_ID = Industry2.MOD_ID;
    private final int[][] machineTexture = new int[][]{
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_trommel.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_trommel_on.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_casing_basic.png")
    };

    public BlockMachineTrommel(String key, int id, Material material) {
    ...
    protected TileEntity getNewBlockEntity() {
        return new TileEntityMachineTrommel();
    }

    @Override
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        if (!world.isClientSide) {
            TileEntityMachineTrommel tileEntity = (TileEntityMachineTrommel) world.getBlockTileEntity(x, y, z);
    ...
    public int getBlockTexture(WorldSource blockAccess, int x, int y, int z, Side side) {
        ...
        */
        ...
        TileEntityMachineTrommel tileEntity = ((TileEntityMachineTrommel) blockAccess.getBlockTileEntity(x, y, z));
    ...
    private static BlockMachineTrommel instance = null;

    private static void setupInstance(BlockMachineTrommel machine) {
        instance = machine;
    }

    private static BlockMachineTrommel getInstance() {
        if (instance == null)
            throw new NullPointerException("Instance of BlockMachineTrommel hasn't been setup!");
        return instance;
    }
