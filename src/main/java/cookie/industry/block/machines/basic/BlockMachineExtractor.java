...
public class BlockMachineExtractor extends BlockTileEntityRotatable {
    private boolean keepInventory = false;
    private final String MOD_ID = Industry2.MOD_ID;
    private final int[][] machineTexture = new int[][]{
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_extractor.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_extractor_on.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_casing_basic.png")
    };

    public BlockMachineExtractor(String key, int id, Material material) {
    ...
    protected TileEntity getNewBlockEntity() {
        return new TileEntityMachineExtractor();
    }

    @Override
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        if (!world.isClientSide) {
            TileEntityMachineExtractor tileEntity = (TileEntityMachineExtractor) world.getBlockTileEntity(x, y, z);
    ...
    public int getBlockTexture(WorldSource blockAccess, int x, int y, int z, Side side) {
        ...
        */
        ...
        TileEntityMachineExtractor tileEntity = ((TileEntityMachineExtractor) blockAccess.getBlockTileEntity(x, y, z));
