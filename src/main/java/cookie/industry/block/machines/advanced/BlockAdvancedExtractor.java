...
import cookie.industry.block.machines.advanced.entity.TIleEntityAdvancedExtractor;
...
public class BlockAdvancedExtractor extends BlockTileEntityRotatable {
    private boolean keepInventory = false;
    private final String MOD_ID = Industry2.MOD_ID;
    private final int[][] machineTexture = new int[][]{
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "advanced_extractor.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "advanced_extractor_on.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_casing_advanced.png")
    };

    public BlockAdvancedExtractor(String key, int id, Material material) {
    ...
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        if (!world.isClientSide) {
            TIleEntityAdvancedExtractor tileEntity = (TIleEntityAdvancedExtractor) world.getBlockTileEntity(x, y, z);
    ...
    public int getBlockTexture(WorldSource blockAccess, int x, int y, int z, Side side) {
        ...
        */
        ...
        TIleEntityAdvancedExtractor tileEntity = ((TIleEntityAdvancedExtractor) blockAccess.getBlockTileEntity(x, y, z));
    ...
    private static BlockAdvancedExtractor instance = null;

    private static void setupInstance(BlockAdvancedExtractor machine) {
        instance = machine;
    }

    private static BlockAdvancedExtractor getInstance() {
        if (instance == null)
            throw new NullPointerException("Instance of BlockAdvancedExtractor hasn't been setup!");
        return instance;
    }
