...
import cookie.industry.block.machines.advanced.entity.TileEntityAdvancedCompressor;
...
public class BlockAdvancedCompressor extends BlockTileEntityRotatable {
    private boolean keepInventory = false;
    private final String MOD_ID = Industry2.MOD_ID;
    private final int[][] machineTexture = new int[][]{
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "advanced_compressor.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "advanced_compressor_on.png"),
            TextureHelper.getOrCreateBlockTexture(MOD_ID, "machine_casing_advanced.png")
    };

    public BlockAdvancedCompressor(String key, int id, Material material) {
    ...
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        if (!world.isClientSide) {
            TileEntityAdvancedCompressor tileEntity = (TileEntityAdvancedCompressor) world.getBlockTileEntity(x, y, z);
    ...
    public int getBlockTexture(WorldSource blockAccess, int x, int y, int z, Side side) {
        ...
        */
        ...
        TileEntityAdvancedCompressor tileEntity = ((TileEntityAdvancedCompressor) blockAccess.getBlockTileEntity(x, y, z));
    ...
    private static BlockAdvancedCompressor instance = null;

    private static void setupInstance(BlockAdvancedCompressor machine) {
        instance = machine;
    }

    private static BlockAdvancedCompressor getInstance() {
        if (instance == null)
            throw new NullPointerException("Instance of BlockAdvancedCompressor hasn't been setup!");
        return instance;
    }
