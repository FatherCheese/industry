...
import cookie.industry.block.machines.advanced.BlockAdvancedMacerator;
import cookie.industry.recipe.RecipesMacerator;
import net.minecraft.core.item.ItemStack;

public class TileEntityAdvancedMacerator extends TileEntityAdvancedBase {
    RecipesMacerator recipes = new RecipesMacerator();

    @Override
    public String getInvName() {
        return "IndustryAdvancedMacerator";
    }
    ...
    public void tick() {
        ...
        if (!worldObj.isClientSide) {
            if (currentMachineTime == 0 || currentMachineTime > 0 && contents[2] != null || contents[3] != null) {
                BlockAdvancedMacerator.updateBlockState(true, worldObj, x, y, z);
                onInventoryChanged();
