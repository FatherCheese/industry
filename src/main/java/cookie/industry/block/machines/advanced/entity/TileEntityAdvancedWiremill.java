...
import cookie.industry.block.machines.advanced.BlockAdvancedWiremill;
import cookie.industry.recipe.RecipesWiremill;
import net.minecraft.core.item.ItemStack;

public class TileEntityAdvancedWiremill extends TileEntityAdvancedBase {
    RecipesWiremill recipes = new RecipesWiremill();

    @Override
    public String getInvName() {
        return "IndustryAdvancedWiremill";
    }
    ...
    public void tick() {
        ...
        if (!worldObj.isClientSide) {
            if (currentMachineTime == 0 || currentMachineTime > 0 && contents[2] != null || contents[3] != null) {
                BlockAdvancedWiremill.updateBlockState(true, worldObj, x, y, z);
                onInventoryChanged();
