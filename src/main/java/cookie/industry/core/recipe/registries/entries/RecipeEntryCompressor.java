...
public class RecipeEntryCompressor extends RecipeEntryBase<RecipeSymbol, ItemStack, Void> {
    public RecipeEntryCompressor(RecipeSymbol input, ItemStack output) {
        super(input, output, null);
    }

    public RecipeEntryCompressor() {
    }
    ...
    private boolean matchesScope(SearchQuery query) {
        if (query.scope.getLeft() == SearchQuery.SearchScope.NONE) return true;
            ...
            if (query.scope.getLeft() == SearchQuery.SearchScope.NAMESPACE) {
                ...
                RecipeGroup<RecipeEntryCompressor> group;
                try {
