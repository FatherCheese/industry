...
public class RecipeEntryMacerator extends RecipeEntryBase<RecipeSymbol, ItemStack, Void> {
    public RecipeEntryMacerator(RecipeSymbol input, ItemStack output) {
        super(input, output, null);
    }

    public RecipeEntryMacerator() {
    }
    ...
    private boolean matchesScope(SearchQuery query) {
        if (query.scope.getLeft() == SearchQuery.SearchScope.NONE) return true;
            ...
            if (query.scope.getLeft() == SearchQuery.SearchScope.NAMESPACE) {
                ...
                RecipeGroup<RecipeEntryMacerator> group;
                try {
