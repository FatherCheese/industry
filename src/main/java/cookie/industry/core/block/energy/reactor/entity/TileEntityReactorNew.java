...
public class TileEntityReactorNew extends TileEntityEnergyConductor implements IInventory {
    ...
    public void onInventoryChanged() {
        ...
        for (ItemStack content : contents) {
            if (content != null) {
                if (content.getItem() == I2ItemsNew.TIER_1_REDSTONE_CELL)
                    uraniumCell += 1;

                if (content.getItem() == I2ItemsNew.COOLANT_CELL)
                    coolantCell += 1;

                if (content.getItem() == I2ItemsNew.REACTOR_PLATE)
                    maxHeat += 250;
            }
        }
    ...
    public boolean isAssembled() {
        ...
        for (Side side : sides) {
            ...
            if (worldObj.getBlockId(_x, y, _z) != I2BlocksNew.REACTOR_CHAMBER.id &&
                    worldObj.getBlockId(x, _y, z) != I2BlocksNew.REACTOR_IO.id)
                return false;
        }
        return true;
    }
    ...
    private boolean isUranium(int id) {
        ...
        return contents[id].getItem() == I2ItemsNew.TIER_1_REDSTONE_CELL;
    }
    ...
    public void tick() {
        ...
        if (!worldObj.isClientSide && isAssembled() && !isDisabled) {
            ...
            for (int i = 0; i < contents.length; i++) {
                ...
                if (damageUranium && stack.getItem() == I2ItemsNew.TIER_1_REDSTONE_CELL) {
                    heat += 4;
                    stack.damageItem(1, null);
                }

                if (damageCoolant && stack.getItem() == I2ItemsNew.COOLANT_CELL) {
