...
public class TileEntityReactorNewer extends TileEntityEnergyConductor implements IInventory {
    ...
    public void onInventoryChanged() {
        ...
        for (int i = 3; i < 51; i++) {
            if (contents[i] != null) {
                if (contents[i].getItem() == I2ItemsNew.TIER_1_REDSTONE_CELL) {
                    redstoneCellsT1 += 1;
                }

                if (contents[i].getItem() == I2ItemsNew.TIER_2_REDSTONE_CELL) {
                    redstoneCellsT2 += 1;
                }

                if (contents[i].getItem() == I2ItemsNew.TIER_3_REDSTONE_CELL) {
                    redstoneCellsT3 += 1;
                }

                if (contents[i].getItem() == I2ItemsNew.REACTOR_VENT) {
                    reactorVents += 1;
                }

                if (contents[i].getItem() == I2ItemsNew.REACTOR_PLATE) {
                    maxHeat += 500;
                }
            }
        }
    ...
    public boolean isAssembled() {
        ...
        for (Side side : sides) {
            ...
            if (worldObj.getBlockId(_x, y, _z) != I2BlocksNew.REACTOR_CHAMBER.id && worldObj.getBlockId(x, _y, z) != I2BlocksNew.REACTOR_IO.id) {
                return false;
            }
        }
        return true;
    ...
    private void overHeat() {
        ...
        worldObj.spawnParticle("smoke", _x, _y + 0.22, _z, 0.0, 0.0, 0.0, 0);
        worldObj.spawnParticle("flame", _x, _y + 0.22, _z, 0.0, 0.0, 0.0, 0);
                    ...
                    if (worldMats == Material.water && heat - 25 * reactorVents > 0) {
                        ...
                        switch (worldObj.rand.nextInt(4)) {
                            ...
                            case 0:
                                ...
                                worldObj.playSoundEffect(null, SoundCategory.WORLD_SOUNDS, x + 1, y - 1, _z, "random.fizz", 0.1f, worldObj.rand.nextFloat() * 0.7F + 0.3F);
                                break;
                            case 1:
                                worldObj.setBlockWithNotify(x - 1, y - 1, _z, 0);
                                worldObj.playSoundEffect(null, SoundCategory.WORLD_SOUNDS, x - 1, y - 1, _z, "random.fizz", 0.1f, worldObj.rand.nextFloat() * 0.7F + 0.3F);
                            ...
                            case 2:
                                ...
                                worldObj.playSoundEffect(null, SoundCategory.WORLD_SOUNDS, _x, y - 1, z + 1, "random.fizz", 0.1f, worldObj.rand.nextFloat() * 0.7F + 0.3F);
        ...
        if (!worldObj.isClientSide && isAssembled()) {
            ...
            for (int i = 3; i < 51; i++) {
                ...
                if (shouldDamageRedstone) {
                    ...
                    if (stacks.getItem() == I2ItemsNew.TIER_1_REDSTONE_CELL) {
                        heat += 5 * redstoneCellsT1;

                        if (stacks.stackSize <= 0) contents[i] = new ItemStack(I2ItemsNew.DEPLETED_TIER_1_REDSTONE_CELL);
                    }

                    if (stacks.getItem() == I2ItemsNew.TIER_2_REDSTONE_CELL) {
                        heat += 25 * redstoneCellsT2;

                        if (stacks.stackSize <= 0) contents[i] = new ItemStack(I2ItemsNew.DEPLETED_TIER_2_REDSTONE_CELL);
                    }

                    if (stacks.getItem() == I2ItemsNew.TIER_3_REDSTONE_CELL) {
