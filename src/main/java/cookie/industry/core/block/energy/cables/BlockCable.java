...
public class BlockCable extends BlockTileEntity {
    ...
    private AABB cableBoundingBox(WorldSource blockAccess, BlockCable blockCable, int x, int y, int z) {
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        ...
        if (entity instanceof EntityLiving && dangerLevel > 0) {
            if (tile.getEnergy() > 0 && tile.getEnergy() - dangerLevel * 10 > 0) {
                tile.modifyEnergy(-dangerLevel * 10);

                entity.hurt(null, dangerLevel, DamageType.GENERIC);

                Random random = new Random();
                double randX = random.nextDouble() + x;
                double randY = random.nextDouble() + y;
                double randZ = random.nextDouble() + z;
                world.spawnParticle("flame", randX, randY, randZ, 0.0, 0.0, 0.0, 0);
                world.playSoundAtEntity(entity, entity, "industry.zap", 0.1f, 0.8f);
            }
        }
    }
