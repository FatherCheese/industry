package cookie.industry.core.block.energy.batteryboxes.entities;
...
public class TileEntityLVBatterybox extends TileEntityMachineBase implements IInventory {
    public TileEntityLVBatterybox() {
        maxProvide = I2Config.cfg.getInt("Energy Values.lvIO");
        maxReceive = I2Config.cfg.getInt("Energy Values.lvIO");
        setCapacity(I2Config.cfg.getInt("Energy Values.lvMachineStorage") * 3);

        for (Direction dir : Direction.values()) {
            if (dir == Direction.Y_POS || dir == Direction.Y_NEG) setConnection(dir, Connection.INPUT);
            else setConnection(dir, Connection.OUTPUT);
        }
    }
