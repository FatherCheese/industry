...
public class BlockLVWatermill extends BlockTileEntity {
    public BlockLVWatermill(String key, int id) {
        super(key, id, Material.metal);
    }

    @Override
    protected TileEntity getNewBlockEntity() {
        return new TileEntityLVWatermill();
    }

    @Override
    public boolean onBlockRightClicked(World world, int x, int y, int z, EntityPlayer player, Side side, double xHit, double yHit) {
        TileEntityLVWatermill tileEntity = (TileEntityLVWatermill) world.getBlockTileEntity(x, y, z);
    ...
    private void dropContents(World world, int x, int y, int z) {
        TileEntityLVWatermill tileEntity = (TileEntityLVWatermill) world.getBlockTileEntity(x, y, z);
        if (!(tileEntity instanceof TileEntityLVWatermill))
    ...
    public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
        switch (dropCause) {
            ...
            case PICK_BLOCK:
            case PROPER_TOOL:
                return new ItemStack[]{new ItemStack(I2BlocksNew.LV_WATERMILL)};
            case IMPROPER_TOOL:
            case WORLD:
