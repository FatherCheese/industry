...
import cookie.industry.core.block.machines.lv.production.entities.TileEntityLVRecycler;
...
public class BlockLVRecycler extends BlockTileEntityRotatable {
    public BlockLVRecycler(String key, int id) {
    ...
    protected TileEntity getNewBlockEntity() {
        return new TileEntityLVRecycler();
    }

    @Override
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        TileEntityLVRecycler tileEntity = (TileEntityLVRecycler) world.getBlockTileEntity(x, y, z);
    ...
    private void dropContents(World world, int x, int y, int z) {
        TileEntityLVRecycler tileEntity = (TileEntityLVRecycler) world.getBlockTileEntity(x, y, z);
        if (!(tileEntity instanceof TileEntityLVRecycler))
    ...
    public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
        switch (dropCause) {
            ...
            case PICK_BLOCK:
            case PROPER_TOOL:
                return new ItemStack[]{new ItemStack(I2BlocksNew.LV_RECYCLER)};
            case IMPROPER_TOOL:
            case WORLD:
