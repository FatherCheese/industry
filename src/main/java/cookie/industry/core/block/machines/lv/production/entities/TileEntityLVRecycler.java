...
public class TileEntityLVRecycler extends TileEntityLVMachineBase implements IItemIO {
    ...
    */
    private boolean canProduce() {
        ...
        ItemStack output = I2ItemsNew.SCRAP.getDefaultStack();

        if (output == null) return false;
        if (slots[2].getItem().hasTag(I2Tags.PREVENT_ITEM_RECYCLING)) return false;
    ...
    */
    private void produceItem() {
        if (canProduce()) {
            ...
            if (rand.nextInt(3) == 0) output = I2ItemsNew.SCRAP.getDefaultStack();

            if (slots[3] == null && output != null) slots[3] = output.copy();
            else if (slots[3] != null && output != null && slots[3].itemID == output.itemID && rand.nextInt(3) == 0)
