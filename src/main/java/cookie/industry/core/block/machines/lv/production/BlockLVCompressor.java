...
public class BlockLVCompressor extends BlockTileEntityRotatable {
    public BlockLVCompressor(String key, int id) {
    ...
    protected TileEntity getNewBlockEntity() {
        return new TileEntityLVCompressor();
    }

    @Override
    public boolean onBlockRightClicked(World world, int x, int y, int z, EntityPlayer player, Side side, double xPlaced, double yPlaced) {
        TileEntityLVCompressor tileEntity = (TileEntityLVCompressor) world.getBlockTileEntity(x, y, z);
    ...
    private void dropContents(World world, int x, int y, int z) {
        TileEntityLVCompressor tileEntity = (TileEntityLVCompressor) world.getBlockTileEntity(x, y, z);
        if (!(tileEntity instanceof TileEntityLVCompressor))
    ...
    public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
        switch (dropCause) {
            ...
            case PICK_BLOCK:
            case PROPER_TOOL:
                return new ItemStack[]{new ItemStack(I2BlocksNew.LV_COMPRESSOR)};
            case IMPROPER_TOOL:
            case WORLD:
